somatrixHz <- function(PrpK,sigma,AdHz){
  ...
  #   with density dependence, potential additive hazards (dens-independ) 
  ...
  # NOTE2: hazard ratios are useful for fitting AdHz to carcass data...
  #    ie ratio of exp(AdHz)/(exp(AdHa)+exp(sum(zeta))) gives expected proportion of carcasses from AdHz
  ...
  fs = exp(-z0*exp(z1+z3*PrpK+AdHz+eps))
  ms = exp(-z0*exp(z1+z2+z3*PrpK+AdHz+eps))
  M = matrix(c(fs*(1+R),   0,
