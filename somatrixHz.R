somatrixHz <- function(PrpK,sigma,AdHzF,AdHzM){
  ...
  #   Sx = exp(-gamma)
  #   gamma = exp(zeta1*sum(zeta2 + zeta3... + AdHz + eps))
  #     zeta1 = intercept (.01), ie. if no hazards 99% survival
  #     zeta2 = shared hazards for females and males when food abundant
  #     zeta3 = additional hazards for males
  #     zeta4 = Density-dependent hazards (scaled by proportion of K)
  ...
  #      (rho parameters scaled to match telmetry estimates and basic logistic growth model,
  ...
  # NOTE3: expected pup count (assuming **60% pups born & still dependent at spring survey):
  #    pups = n[1,t-1]*fs*R*1.2
  #
  # OUTPUT:
  ...
  zeta1 = .01 # Intercept (nuiscence parameter)
  zeta2 = 1.3	# Shared hazards for all animals when density low, food abundant
  zeta3 = 0.5  # Additional hazards for males **ALLOW THIS TO BE FIT TO DATA?**
  zeta4 = 1.5  # Added density-dependent hazards (scaled by proportion of K)
  rho1 = 0.3 # Param for reproduction
  rho2 = 2  # Param for reproduction
  rho3 = -3 # Param for reproduction
  ...
  ms = exp(-zeta1*exp(zeta2+zeta3+zeta4*PrpK+AdHzM+eps))
  M = matrix(c(fs*(1+R),   0,
